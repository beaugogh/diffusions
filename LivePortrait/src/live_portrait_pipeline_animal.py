...
class LivePortraitPipelineAnimal(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        wfp_concat = osp.join(args.output_dir, f'{basename(args.source)}--{basename(args.driving)}-animal_concat.mp4')
        images2video(frames_concatenated, wfp=wfp_concat, fps=output_fps)

        if flag_driving_has_audio:
            # final result with concatenation
            wfp_concat_with_audio = osp.join(args.output_dir, f'{basename(args.source)}--{basename(args.driving)}-animal_concat_with_audio.mp4')
        ...
        wfp = osp.join(args.output_dir, f'{basename(args.source)}--{basename(args.driving)}-animal.mp4')
        ...
        if flag_driving_has_audio:
            wfp_with_audio = osp.join(args.output_dir, f'{basename(args.source)}--{basename(args.driving)}-animal_with_audio.mp4')
            audio_from_which_video = args.driving
