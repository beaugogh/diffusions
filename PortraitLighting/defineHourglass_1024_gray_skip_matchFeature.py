...
class BasicBlock(nn.Module):
    def __init__(
        self, inplanes, outplanes, batchNorm_type=0, stride=1, downsample=None
    ):
...
class HourglassBlock(nn.Module):
    ...
    def forward(self, x, light, count, skip_count):
        ...
        out_lower, out_feat, out_middle = self.middle(
...
class lightingNet(nn.Module):
    ...
    def __init__(self, ncInput, ncOutput, ncMiddle):
        ...
        self.predict_FC1 = nn.Conv2d(
            self.ncInput, self.ncMiddle, kernel_size=1, stride=1, bias=False
        )
        self.predict_relu1 = nn.PReLU()
        self.predict_FC2 = nn.Conv2d(
            self.ncMiddle, self.ncOutput, kernel_size=1, stride=1, bias=False
        )

        self.post_FC1 = nn.Conv2d(
            self.ncOutput, self.ncMiddle, kernel_size=1, stride=1, bias=False
        )
        self.post_relu1 = nn.PReLU()
        self.post_FC2 = nn.Conv2d(
            self.ncMiddle, self.ncInput, kernel_size=1, stride=1, bias=False
        )
...
class HourglassNet(nn.Module):
    ...
    def __init__(self, baseFilter=16, gray=True):
        ...
        self.conv_1 = nn.Conv2d(
            self.ncPre, self.ncPre, kernel_size=3, stride=1, padding=1
        )
        self.bn_1 = nn.BatchNorm2d(self.ncPre)
        self.conv_2 = nn.Conv2d(
            self.ncPre, self.ncPre, kernel_size=1, stride=1, padding=0
        )
...
class HourglassNet_1024(nn.Module):
    ...
    def forward(self, x, target_light, skip_count, oriImg=None):
        ...
        out_feat, out_light, out_feat_ori, feat = self.model_512(
            feat, target_light, skip_count, oriImg
        )
